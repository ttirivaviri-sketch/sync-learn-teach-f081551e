...
export const LearnerProfileTab = ({
  ...
  return (
    <div className="px-5 pt-6 pb-28 space-y-6">
      ...
      {/* ── Academic profile summary — always editable in one tap ── */}
      <div className="rounded-2xl border border-border bg-card p-4 shadow-sm">
        <div className="flex items-start justify-between gap-3">
              ...
              {academicProfile
                ? [
                    ...
                    academicProfile.subjects?.length
                      ? `${academicProfile.subjects.length} subjects`
                      : null,
                    academicProfile.exam_year ? `Exams ${academicProfile.exam_year}` : null,
                  ]
                    .filter(Boolean)
                    .join(" · ")
                : "Add your curriculum, grade and subjects to personalise everything."}
            </p>
            {academicProfile?.subjects?.length ? (
              <div className="mt-2 flex flex-wrap gap-1">
                {academicProfile.subjects.slice(0, 6).map((s) => (
                  <span
                    key={s}
                ...
                {academicProfile.subjects.length > 6 && (
          ...
          <button
            onClick={onShowAcademicSetup}
            className="shrink-0 rounded-full bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow-sm active:opacity-90"
          >
            {academicProfile ? "Edit" : "Set up"}
          </button>
        </div>
      </div>
