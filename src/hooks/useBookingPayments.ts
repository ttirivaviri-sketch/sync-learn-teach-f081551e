...
export const useBookingPayments = (bookingIds: string[]) => {
  const { isDevMode, bypassPayments } = useDevMode();
  const [paymentStatuses, setPaymentStatuses] = useState<Record<string, PaymentStatus>>({});
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // In dev mode with bypass, treat all bookings as paid
    if (isDevMode && bypassPayments) {
      const statusMap: Record<string, PaymentStatus> = {};
      bookingIds.forEach(id => {
        statusMap[id] = { bookingId: id, status: 'succeeded', paymentId: `dev-pay-${id}` };
      });
  ...
  }, [JSON.stringify(bookingIds), isDevMode, bypassPayments]);

  const needsPayment = (bookingId: string) => {
    if (isDevMode && bypassPayments) return false;
  ...
  const isPaid = (bookingId: string) => {
    if (isDevMode && bypassPayments) return true;
    return paymentStatuses[bookingId]?.status === 'succeeded';
  };

  return { paymentStatuses, loading, needsPayment, isPaid };
};
