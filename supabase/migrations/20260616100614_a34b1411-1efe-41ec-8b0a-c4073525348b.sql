...
BEGIN
  ...
  v_day := COALESCE(NEW.created_at, now())::date;
  v_score := COALESCE(NEW.marks_awarded, CASE WHEN NEW.was_correct THEN 1 ELSE 0 END);
  v_max := COALESCE(NEW.marks_possible, 1);
