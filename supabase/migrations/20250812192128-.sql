...
CREATE POLICY "Participants and admin can view bookings" ON public.bookings
FOR SELECT USING (
...
DROP POLICY IF EXISTS "Participants and admin can update bookings" ON public.bookings;
CREATE POLICY "Participants and admin can update bookings" ON public.bookings
FOR UPDATE USING (
  auth.uid() = learner_id OR auth.uid() = tutor_id
  OR has_role(auth.uid(), 'admin'::app_role)
);
...
CREATE POLICY "Only admin can delete bookings" ON public.bookings
FOR DELETE USING (has_role(auth.uid(), 'admin'::app_role));

-- Enforce limited update surface for non-admin: only status may change
...
-- Policies: payments
DROP POLICY IF EXISTS "Payer and admin can view payments" ON public.payments;
CREATE POLICY "Payer and admin can view payments" ON public.payments
FOR SELECT USING (
  auth.uid() = payer_id OR has_role(auth.uid(), 'admin'::app_role)
);
...
DROP POLICY IF EXISTS "Ticket stakeholders and admin can view" ON public.support_tickets;
CREATE POLICY "Ticket stakeholders and admin can view" ON public.support_tickets
FOR SELECT USING (
  auth.uid() = creator_id OR auth.uid() = assignee_id OR has_role(auth.uid(), 'admin'::app_role)
);
...
DROP POLICY IF EXISTS "Assignees and admin can update tickets" ON public.support_tickets;
CREATE POLICY "Assignees and admin can update tickets" ON public.support_tickets
FOR UPDATE USING (auth.uid() = assignee_id OR has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Only admin can delete tickets" ON public.support_tickets;
CREATE POLICY "Only admin can delete tickets" ON public.support_tickets
FOR DELETE USING (has_role(auth.uid(), 'admin'::app_role));

-- Policies: verification_reviews
DROP POLICY IF EXISTS "Only admin can view verification reviews" ON public.verification_reviews;
CREATE POLICY "Only admin can view verification reviews" ON public.verification_reviews
FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Only admin can insert verification reviews" ON public.verification_reviews;
CREATE POLICY "Only admin can insert verification reviews" ON public.verification_reviews
FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Only admin can update verification reviews" ON public.verification_reviews;
CREATE POLICY "Only admin can update verification reviews" ON public.verification_reviews
FOR UPDATE USING (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Only admin can delete verification reviews" ON public.verification_reviews;
CREATE POLICY "Only admin can delete verification reviews" ON public.verification_reviews
FOR DELETE USING (has_role(auth.uid(), 'admin'::app_role));
