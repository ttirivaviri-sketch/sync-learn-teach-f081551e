CREATE UNIQUE INDEX IF NOT EXISTS school_memberships_school_user_role_uniq
  ON public.school_memberships (school_id, user_id, role)
  WHERE user_id IS NOT NULL;

ALTER TABLE public.school_memberships
  DROP CONSTRAINT IF EXISTS school_memberships_school_user_role_key;

-- Recreate the accept RPC using the partial unique index target columns
...
BEGIN
  ...
  ON CONFLICT (school_id, user_id, role) WHERE user_id IS NOT NULL
  DO UPDATE SET status='active',
                joined_at=COALESCE(public.school_memberships.joined_at, now()),
                updated_at=now();
